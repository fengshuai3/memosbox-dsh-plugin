...
describe('release workflow and end-to-end gates', () => {
  it('defaults to verification and separates the privileged publication job', () => {
    ...
    expect(commands.indexOf('release-check.mjs')).toBeLessThan(commands.indexOf('gh release create'))
    expect(commands).toContain('--verify-tag --prerelease --latest=false')
    expect(commands).toContain('"$MEMOSBOX_RELEASE_ARTIFACT" artifacts/release/SHA256SUMS')
    expect(commands).not.toContain('npm publish')
    const publishStep = workflow.jobs.publish.steps.find((step: { run?: string }) => step.run?.includes('gh release create'))
